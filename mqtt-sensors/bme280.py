...
def main():
    ...
    try:
        ...
        logging.error("bme280 reader failed to open mqtt connection. {}".format(e))
        sys.exit(1)
    mqttc.loop_start()
    ...
    while True:
        ...
        mqttc.publish(TEMPERATURE_TOPIC, data['temperature'])
        mqttc.publish(HUMIDITY_TOPIC, data['humidity'])
        mqttc.publish(PRESSURE_TOPIC, data['pressure'])
        logging.info("messages published from data: {}".format(data))
        print("messages published from data: {}".format(data))
